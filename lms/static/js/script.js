...
function createCircle(data, index) {
    ...
    circle.addEventListener("click", () => {
        ...
        if (wrapper) {
            ...
            if (isCompleted) wrapper.classList.add("completed-frame");
        }
    });

    circlesWrapper.appendChild(circle);
    circleElements.push({ index, element: circle });
}
...
function updateEvalStatusInRoot(root, evalData) {
    ...
    function setIcon(selector, statusKey) {
        ...
        img.src = icons[status === "completed" ? "completed" : status];
    ...
    setIcon(".eval-assignment", "assignment");
    setIcon(".eval-pre", "pre");
    setIcon(".eval-post", "post");

    const chLi = root.querySelector(".eval-challenges");
    if (!chLi || !evalData.challenges) return;

    const { completed, total, status } = evalData.challenges;
    const labels = chLi.querySelectorAll(".label");
    if (labels.length > 0) labels[0].textContent = `${completed}/${total}`;
